...
class TestSearchCode:
    ...
    def test_it_should_search_on_gl(self, mock_search_in_repo: Mock) -> None:
        repo_name_1 = "test_1"
        repo_name_2 = "test_2"
        repo_name_3 = "test_3"
        web_url_1 = "url1"
        ...
        response = _search_code(repo_list, "test", 5)
    ...
    def test_mx_workers(self, mock_thread_pool_executor: Mock, max_workers: int) -> None:
        _search_code([], "test", max_workers)
...
class TestSearchInRepo:
    ...
    def test_check_url(self, mock_retrieve_data: Mock) -> None:
        search_param = "test"
        repo_id = 1

        _search_in_repo(SearchRepoParams(repo_id=repo_id, search=search_param))

        request_describe = RequestDescribe(
            url=f"https://gitlab.com/api/v4/projects/{repo_id}/search",
            params={"scope": "blobs", "search": search_param},
        )
        mock_retrieve_data.assert_called_with(request_describe, ANY)

    @patch("requests.get")
    def test_it_should_return_sentry_entry_result_list(self, mock_requests_get: Mock) -> None:
        ...
        response = _search_in_repo(SearchRepoParams(repo_id=1, search="test"))
        assert response == [
            SearchEntryResult(
...
class TestRetrieveInformationFromRepositoriesOfEachGroup:
    ...
    def test_it_should_search_each_repo_on_the_list(self, mock_retrieve_repositories_by: Mock) -> None:
        _retrieve_information_from_repositories_of_each_group([1, 2, 3], 1, [])
        assert mock_retrieve_repositories_by.call_args_list == [
            call(1, visibility_choose=[]),
            call(2, visibility_choose=[]),
            call(3, visibility_choose=[]),
        ]

    @patch("gl_search.search._retrieve_repositories_by")
    def test_visibility_choose(self, mock_set_repos_by_group: Mock) -> None:
        _retrieve_information_from_repositories_of_each_group([1], 1, ["public"])
        assert mock_set_repos_by_group.call_args_list == [
            call(1, visibility_choose=["public"]),
        ]

    @patch("gl_search.search._retrieve_repositories_by")
    def test_repo_info_should_be_unique(self, mock_retrieve_repositories_by: Mock) -> None:
        mock_retrieve_repositories_by.side_effect = [{1, 2, 3}, {3, 4, 5}, {4, 5, 6}]
        repo_set = _retrieve_information_from_repositories_of_each_group([1, 2, 3], 1, [])
        assert mock_retrieve_repositories_by.call_args_list == [
            call(1, visibility_choose=[]),
            call(2, visibility_choose=[]),
            call(3, visibility_choose=[]),
    ...
    @patch("concurrent.futures.ThreadPoolExecutor")
    def test_mx_workers(self, mock_thread_pool_executor: Mock, max_workers: int) -> None:
        _retrieve_information_from_repositories_of_each_group([1, 2, 3], max_workers, [])
        mock_thread_pool_executor.assert_called_with(max_workers=max_workers)
...
class TestRetrieveRepositoriesBy:
    ...
    def test_it_should_retrieve_group_and_each_visibility(
        self, mock_retrieve_data: Mock, group_id: int, visibilities: list[str]
    ) -> None:

        _retrieve_repositories_by(group_id, visibilities)
    ...
    @patch("requests.get")
    def test_it_should_return_repo_item(self, mock_requests_get: Mock) -> None:
        mock_requests_get.return_value = build_response(
            HTTPStatus.OK,
            [
        ...
        response: set[Repo] = _retrieve_repositories_by(1, ["private"])
...
class TestRetrieveGroupsIds:
    @patch("gl_search.search.retrieve_data")
    def test_it_should_retrieve_from_gl(self, mock_retrieve_data: Mock) -> None:
        _retrieve_groups_ids()
    ...
    @patch("requests.get")
    def test_it_should_return_int_list(self, mock_requests_get: Mock) -> None:
        mock_requests_get.return_value = build_response(HTTPStatus.OK, [{"id": 1}, {"id": 2}])
        response: list[int] = _retrieve_groups_ids()
...
class TestSearch:
    ...
    def test_call_methods(
        ...
        mock_retrieve_information_from_repositories_of_each_group.assert_called_once_with(
            [1, 2], 1, ["public"]
        )
